...
export default function EditPackageClient({ initialPackage }: Props) {
  ...
  const [form, setForm] = useState({
    package_code: initialPackage.package_code ?? '',
    name: initialPackage.name ?? '',
    description: initialPackage.description ?? '',
    price: initialPackage.price ? String(initialPackage.price) : '',
    is_active: initialPackage.is_active ?? true,
    display_order: initialPackage.display_order ?? 0,
    image_drive_file_id: initialPackage.image_drive_file_id ?? '',
    image_url: initialPackage.image_drive_file_id ? buildDriveImageUrl(initialPackage.image_drive_file_id) : (initialPackage.image_url ?? ''),
    image_filename: initialPackage.image_filename ?? '',
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/packages/${initialPackage.id}`, {
        method: 'PUT',
  ...
  const handleDelete = async () => {
    if (!confirm(`Hapus paket "${initialPackage.name}" secara permanen?`)) return
    setDeleting(true)
    try {
      const res = await fetch(`/api/admin/packages/${initialPackage.id}`, {
        method: 'DELETE',
    ...
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display font-bold text-2xl text-gray-900">Edit Paket Promo</h1>
            <p className="text-gray-500 text-sm">{initialPackage.package_code} — {initialPackage.name}</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <label className="btn-primary cursor-pointer inline-flex items-center gap-2 px-4 py-2.5 text-sm">
                ...
                {uploadingImage ? 'Uploading...' : 'Ganti Gambar Paket'}
        ...
        >
          ...
          {loading ? 'Menyimpan Perubahan...' : 'Simpan Perubahan Paket'}
        </button>
      </form>
    </div>
  )
