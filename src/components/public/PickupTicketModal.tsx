...
import { Ticket, X, Download, Printer, Check, Copy, Store, MapPin, Sparkles } from 'lucide-react'
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 750" width="600" height="750" style="background-color: #ffffff; font-family: Arial, sans-serif;">
  ...
  <text x="35" y="52" fill="#D4AF37" font-size="18" font-weight="bold" letter-spacing="1">TIKET RESMI PENGAMBILAN STAND</text>
  ...
  <text x="50" y="208" fill="#374151" font-size="13">NAMA: ${escName} (Stambuk: ${escStambuk})</text>
  ...
  <rect x="35" y="590" width="530" height="110" fill="#FEF3C7" stroke="#F59E0B" stroke-width="1.5" rx="10" />
  <text x="50" y="620" fill="#92400E" font-size="13" font-weight="bold">&#x1F4CD; LOKASI PENGAMBILAN:</text>
  ...
  const handleExportSVG = async () => {
    try {
      ...
      link.href = url
      link.download = `Tiket-Stand-${orderNumber}.svg`
  ...
  const handleExportPDF = async () => {
    ...
    try {
      // 1. Fetch QR & Logo base64 data URLs for clean offline SVG canvas rendering
      ...
      // 2. Render SVG onto high-resolution offscreen canvas
      ...
      pdf.save(`Tiket-Stand-${orderNumber}.pdf`)
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      ...
      <div className="bg-white rounded-3xl max-w-xl w-full flex flex-col overflow-hidden shadow-2xl border border-gray-200 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-300 uppercase tracking-widest bg-amber-400/10 px-2 py-0.5 rounded-full border border-amber-400/20">
                <Sparkles className="w-3 h-3" /> E-Voucher Resmi
              </div>
              <h3 className="font-display font-bold text-lg text-white">
                Tiket Pengambilan Stand Bazar
              </h3>
            </div>
          </div>
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-xl bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition-all print:hidden"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 sm:p-6 bg-gray-100/80 flex flex-col items-center">
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-gray-100 pb-4">
              <div>
                <span className="text-[10px] font-bold text-emerald-800 uppercase tracking-wider font-display bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-200">
                  Status: Siap Diambil di Stand
                </span>
                <div className="text-xl sm:text-2xl font-black text-gray-900 font-mono mt-2 flex items-center gap-2">
                  <span>{orderNumber}</span>
                  <button onClick={copyOrderNo} className="text-gray-400 hover:text-gray-700 print:hidden">
              ...
              <div className="flex flex-col items-center shrink-0">
                ...
                <span className="text-[9px] font-bold text-gray-500 mt-1 font-mono">SCAN DI STAND</span>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-200/80 grid grid-cols-2 gap-3 text-xs">
              <div>
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Nama Alumni:</span>
              ...
              <div>
                ...
                <span className="text-gray-500 text-[11px]">Metode: Ambil di Stand</span>
              </div>
            </div>
            ...
            <div>
              <span className="text-[11px] font-bold text-gray-700 uppercase tracking-wider font-display block mb-2">
                Rincian Barang Kit yang Wajib Diterima:
              </span>
        ...
        <div className="p-4 border-t border-gray-200 bg-white flex flex-wrap gap-2.5">
          <button
            onClick={onClose}
            className="px-4 py-2.5 rounded-xl border border-gray-300 text-gray-700 font-display font-semibold text-xs hover:bg-gray-100 print:hidden"
          >
            Tutup
          </button>
          ...
          <button
            ...
            className="flex-1 py-2.5 px-4 rounded-xl border border-emerald-800 text-emerald-950 bg-emerald-50 hover:bg-emerald-100 font-display font-bold text-xs flex items-center justify-center gap-2 transition-all print:hidden disabled:opacity-50"
          >
            <Download className="w-4 h-4 text-emerald-800" />
            <span>{isExportingPdf ? 'Mengunduh PDF...' : 'Unduh Tiket PDF'}</span>
          </button>

          <button
            onClick={handlePrint}
            className="py-2.5 px-5 rounded-xl text-white font-display font-bold text-xs flex items-center justify-center gap-2 shadow-sm hover:opacity-95 transition-all print:hidden"
